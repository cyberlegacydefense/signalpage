...
import { SUBSCRIPTION_TIERS, BILLING_INTERVALS, type BillingInterval } from '@/lib/stripe';

function PricingContent() {
  const searchParams = useSearchParams();
  const canceled = searchParams.get('checkout') === 'canceled';
  const [loading, setLoading] = useState(false);
  const [billingInterval, setBillingInterval] = useState<BillingInterval>('quarterly');
  ...
  const currentPricing = BILLING_INTERVALS[billingInterval];
          ...
          <div className="grid gap-8 md:grid-cols-2">
            ...
            <div className="rounded-2xl border-2 border-blue-600 p-8 relative">
              ...
              <div className="mt-4 flex items-baseline">
                {billingInterval === 'quarterly' ? (
                  <>
                    <span className="text-4xl font-bold text-gray-900">${BILLING_INTERVALS.quarterly.pricePerMonth}</span>
                    <span className="ml-1 text-gray-500">/month</span>
                    <span className="ml-2 text-sm text-gray-400 line-through">${BILLING_INTERVALS.monthly.price}</span>
                  </>
                ) : (
                  <>
              ...
              <p className="mt-2 text-sm text-gray-500">
                {billingInterval === 'quarterly'
                  ? `$${BILLING_INTERVALS.quarterly.price} billed every 3 months`
                  : 'Billed monthly'
                }
              </p>
              <p className="mt-4 text-sm text-gray-600">
